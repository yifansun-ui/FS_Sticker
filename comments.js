...
(function () {
  ...
  const POSTED_FILE = "comments-posted.json";
  ...
  function injectStyles() {
    const css = `
      ...
      #lp-comments-toolbar button.send { background: #16a34a; }
      #lp-comments-toolbar button.send:hover { background: #15803d; }
      ...
      .lp-comment-popup button.send { background: #16a34a; }
      .lp-comment-popup button.send:hover { background: #15803d; }
  ...
  function setupUI() {
    ...
    sendBtn.addEventListener("click", () => {
      const unsent = Object.entries(loadAll()).filter(([, c]) => !c.sent);
      if (!unsent.length) {
        alert("送信待ちのコメントがありません");
        return;
      }
    ...
    function render() {
      ...
      const all = loadAll();
      const unsent = Object.entries(all).filter(([, c]) => !c.sent);
      sendBtn.textContent = `📤 送信待ち（${unsent.length}件）`;

      const docHeight = document.documentElement.scrollHeight;
      const docWidth = document.documentElement.scrollWidth;

      // ローカルコメント
      Object.entries(all).sort((a, b) => a[1].createdAt - b[1].createdAt).forEach(([id, c], i) => {
        const pin = document.createElement("div");
        pin.className = "lp-comment-pin" + (c.sent ? " sent" : "");
        pin.style.left = ((c.x / 100) * docWidth) + "px";
        pin.style.top = ((c.y / 100) * docHeight) + "px";
        pin.innerHTML = `<span>${i + 1}</span>`;
        pin.addEventListener("click", (e) => {
          e.stopPropagation();
          showPopup(pin, id, c, true);
        });
        document.body.appendChild(pin);
      });

      // 投稿済みコメント
      postedComments.forEach(({ x, y, text, name, createdAt }, i) => {
        const pin = document.createElement("div");
        pin.className = "lp-comment-pin sent";
